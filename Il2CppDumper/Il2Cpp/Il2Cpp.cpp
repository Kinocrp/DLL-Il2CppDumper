...
static std::string dump_method(Il2CppClass* klass) {
    ...
    while (auto method = IL2CPP::API::il2cpp_class_get_methods(klass, &iter)) {
        if (method->methodPointer) {
            outPut << "\t// RVA: 0x";
            outPut << std::hex << (uint64_t)method->methodPointer - (uint64_t)IL2CPP::hGameAssembly;
            outPut << " VA: 0x";
...
namespace IL2CPP {
    ...
    namespace API {
        ...
        #include "src/il2cpp-api-functions.h"
        #undef DO_API
    }
    ...
    bool Init() {
        ...
        #define DO_API(r, n, p) API::n = (r (*) p)GetProcAddress(hGameAssembly, #n)
        #include "src/il2cpp-api-functions.h"
        #undef DO_API

        LOG("[+] GameAssembly: 0x%llX\n", (uintptr_t)IL2CPP::hGameAssembly);
        return true;
    }

    Il2CppThread* Attach() {
        if (!hGameAssembly) return nullptr;
        if (!API::il2cpp_is_vm_thread(nullptr)) return nullptr;
        if (!domain) {
            domain = API::il2cpp_domain_get();
            LOG("[+] IL2CPP Domain: 0x%llX\n", (uintptr_t)domain);
        }

        auto thread = API::il2cpp_thread_attach(domain);
        LOG("[+] Thread Attach: 0x%llX\n", (uintptr_t)thread);
        return thread;
    }
    ...
    void Dump() {
        ...
        if (API::il2cpp_image_get_class) {
            LOG("[*] Version is greater than 2018.3\n");
            for (int i = 0; i < size; ++i) {
                auto image = API::il2cpp_assembly_get_image(assemblies[i]);
            ...
            LOG("[*] Version is less than 2018.3\n");
            ...
            if (assemblyLoad && assemblyLoad->methodPointer) {
                LOG("[+] Assembly::Load: 0x%llX\n", (uintptr_t)assemblyLoad->methodPointer);
            } else {
                LOG("[-] Miss Assembly::Load\n");
                return;
            }
            if (assemblyGetTypes && assemblyGetTypes->methodPointer) {
                LOG("[+] Assembly::GetTypes: 0x%llX\n", (uintptr_t)assemblyGetTypes->methodPointer);
            } else {
                LOG("[-] Miss Assembly::GetTypes\n");
                return;
            }
