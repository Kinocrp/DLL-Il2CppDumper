...
std::string dump_method(Il2CppClass* klass) {
    ...
    while (auto method = il2cpp_class_get_methods(klass, &iter)) {
        if (method->methodPointer) {
            outPut << "\t// RVA: 0x";
            outPut << std::hex << (uint64_t)method->methodPointer - (uint64_t)hGameAssembly;
            outPut << " VA: 0x";
...
void MainThread(HMODULE hModule) {
    ...
    LOG("[+] Il2CppDumper By Kinocrp.\n");
    LOG("[+] Waiting for GameAssembly.dll.\n");

    while (hGameAssembly == NULL) {
        hGameAssembly = GetModuleHandleA("GameAssembly.dll");
        Sleep(100);
    }

    il2cpp_api_init(hGameAssembly);

    LOG("[+] GameAssembly.dll found at: %p.\n", hGameAssembly);
    LOG("[+] Waiting for il2cpp_init...\n");
